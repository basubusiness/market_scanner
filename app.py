...
def fetch_justetf_chart(isin):
    ...
    try:
        ...
        if len(df["Close"].dropna()) < 30:
            return pd.DataFrame()
        # Patch today's row with live exchange price
        live_price = fetch_justetf_live_price(isin)
...
def render_deepdive(budget):
    ...
    if not yf_sym_raw or yf_sym_raw in ("nan","None",""):
        yf_sym_raw = ticker
    # Only replace dot with dash for class shares like BRK.B → BRK-B
    # European suffixes like .DE .AS .L must stay as dots
    import re as _re
    if _re.match(r'^[A-Z]{1,4}\.[A-Z]$', yf_sym_raw):
        yf_link_sym = yf_sym_raw.replace(".", "-")
    else:
        yf_link_sym = yf_sym_raw
